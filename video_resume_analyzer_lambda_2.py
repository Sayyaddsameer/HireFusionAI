...
def lambda_handler(event, context):
    ...
    try:
        ...
        happy_confidences = [e['Confidence'] for face_item in face_detections 
        ...
        gesture_confidences = [face_item['Face']['Smile']['Confidence'] 
        ...
        print(f"[DEBUG] Waiting for Transcribe Job={analysis_id}")
        for _ in range(6):
            transcribe_result = transcribe.get_transcription_job(TranscriptionJobName=analysis_id)
        ...
        print(f"[SUCCESS] Analysis saved for {analysis_id}")
        return {
            'statusCode': 200,
            'body': json.dumps({'message': 'Analysis saved', 'analysisId': analysis_id})
        }
